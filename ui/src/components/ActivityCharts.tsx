...
const priorityColors = priorityHex;
...
const statusColors = issueStatusHex;
const statusLabels = issueStatusLabel;
    ...
    <div>
                ...
                >
                  {statusOrder.map((s) =>
                    (entry[s] ?? 0) > 0 ? (
                      <div
                        key={s}
                        style={{ flex: entry[s], backgroundColor: statusColors[s] ?? issueStatusHexDefault }}
                      />
                    ) : null,
                  )}
                </div>
      ...
      <ChartLegend
        items={statusOrder.map((s) => ({
          color: statusColors[s] ?? issueStatusHexDefault,
