...
export async function runDatabaseBackup(opts: RunDatabaseBackupOptions): Promise<RunDatabaseBackupResult> {
  ...
  // Create the backup file upfront so we can stream directly to disk instead of
  // buffering all SQL into a single JS string (which crashes with RangeError when
  // the database exceeds ~250 MB — V8 string length limit).
  mkdirSync(opts.backupDir, { recursive: true });
  const backupFile = resolve(opts.backupDir, `${filenamePrefix}-${timestamp()}.sql`);
  const fileStream = createWriteStream(backupFile, { encoding: "utf8" });
  ...
  try {
    ...
    // Destroy the stream and remove the partial backup file on failure
    fileStream.destroy();
    try {
