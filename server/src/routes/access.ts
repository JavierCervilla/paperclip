...
function toInviteSummaryResponse(
  ...
  const companyInfo = typeof company === "string" ? { name: company, brandColor: null, logoUrl: null } : company;
  ...
  return {
    ...
    skillIndexPath,
    skillIndexUrl: baseUrl ? `${baseUrl}${skillIndexPath}` : skillIndexPath,
    inviteMessage,
  };
}
...
async function loadCompanyMemberRecords(db: Db, companyId: string, options: { includeArchived?: boolean } = {}) {
  ...
  return members.map((member) => ({
    ...
    membershipRole: member.membershipRole ? normalizeHumanRole(member.membershipRole, "operator") : null,
...
async function getProtectedMemberReason(
  ...
  const targetRole = member.membershipRole ? normalizeHumanRole(member.membershipRole, "operator") : "operator";
...
async function addCompanyMemberRemovalAccess(
  ...
  const userIds = [
    ...new Set(members.filter((member) => member.principalType === "user").map((member) => member.principalId)),
  ];
...
async function loadCompanyInviteRecords(
  ...
  const userIds = [
    ...new Set(visibleRows.map((invite) => invite.invitedByUserId).filter((value): value is string => Boolean(value))),
  ];
...
async function loadJoinRequestRecords(db: Db, companyId: string) {
  ...
  const inviteRows = inviteIds.length ? await db.select().from(invites).where(inArray(invites.id, inviteIds)) : [];
  const userIds = [
    ...new Set(
      [
...
async function loadUserCompanyAccessResponse(db: Db, access: ReturnType<typeof accessService>, userId: string) {
...
function mergeInviteDefaults(
  ...
  if (humanRole) {
    const existingHuman = isPlainObject(merged.human) ? { ...(merged.human as Record<string, unknown>) } : {};
    merged.human = {
      ...existingHuman,
      role: humanRole,
...
async function resolveAcceptedInviteJoinRequest(db: Db, req: Request, invite: typeof invites.$inferSelect | null) {
  ...
  const actorRequestingUserId = isLocalImplicit(req) ? "local-board" : (req.actor.userId ?? null);
...
export function setInviteResolutionNetworkForTest(network: Partial<InviteResolutionNetwork> | null) {
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timeout = setTimeout(
          () => reject(badRequest(`url hostname DNS lookup timed out after ${INVITE_RESOLUTION_DNS_TIMEOUT_MS}ms`)),
...
export function accessRoutes(
  ...
  async function createCompanyInviteForCompany(input: {
    ...
    const effectiveHumanRole = input.allowedJoinTypes === "agent" ? null : (input.humanRole ?? "operator");
    const insertValues = {
  ...
  router.post("/invites/:token/accept", validate(acceptInviteSchema), async (req, res) => {
    ...
    if (requestType === "human" && actorHasActiveUserMembership(req, companyId)) {
  ...
  router.post("/companies/:companyId/join-requests/:requestId/approve", async (req, res) => {
    ...
    if (existing.requestType === "human") {
      ...
      const membershipRole = resolveHumanInviteRole(invite.defaultsPayload as Record<string, unknown> | null);
  ...
  router.get("/admin/users", async (req, res) => {
    ...
    const adminIds = new Set(
      await Promise.all(userIds.map(async (userId) => ((await access.isInstanceAdmin(userId)) ? userId : null))).then(
