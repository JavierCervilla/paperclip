...
describe.sequential("plugin install and upgrade authz", () => {
  ...
  it("rejects plugin installation for non-admin board users", async () => {
    ...
    const res = await request(app).post("/api/plugins/install").send({ packageName: "paperclip-plugin-example" });
  ...
  it("allows instance admins to upgrade plugins", async () => {
    ...
    const res = await request(app).post(`/api/plugins/${pluginId}/upgrade`).send({ version: "1.1.0" });
