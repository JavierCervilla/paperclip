...
async function createApp(routeType: "project" | "goal") {
  ...
  if (routeType === "project") {
    ...
    const { goalRoutes } = await vi.importActual<typeof import("../routes/goals.js")>("../routes/goals.js");
...
describe("project and goal telemetry routes", () => {
  ...
  it("emits telemetry when a project is created", async () => {
    ...
    const res = await request(app).post("/api/companies/company-1/projects").send({ name: "Telemetry project" });
